...
export default class Engine {
    ...
    tick = () => {
            ...
            for (const target of physicsObjects) {
                ...
                if (!!distance) {
                    ...
                    let overlap =
                        0.5 *
                        (distance -
                            current.RigidBody.radius -
                            target.RigidBody.radius)

                    // displace current ball away from collision
                    current.position.x -=
                        (overlap * (current.position.x - target.position.x)) /
                        distance
                    current.position.y -=
                        (overlap * (current.position.y - target.position.y)) /
                        distance
        ...
        for (const pair of collidingPairs) {
            ...
            // Dot Product Tangent
            let dpTan1 =
                b1.RigidBody.velocity.x * tx + b1.RigidBody.velocity.y * ty
            let dpTan2 =
                b2.RigidBody.velocity.x * tx + b2.RigidBody.velocity.y * ty

            // Dot Product Normal
            let dpNorm1 =
                b1.RigidBody.velocity.x * nx + b1.RigidBody.velocity.y * ny
            let dpNorm2 =
                b2.RigidBody.velocity.x * nx + b2.RigidBody.velocity.y * ny
    ...
    checkCollision(ball0, ball1) {
        ...
        // console.log(ball1.x)
        if (dist < ball0.Renderer.radius + ball1.Renderer.radius) {
            return dist
        }
        // console.log(false)
        return false
